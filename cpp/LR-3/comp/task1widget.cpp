    ...
    : QWidget(parent)
{
    ...
    connect(convertBtn, &QPushButton::clicked, this, &Task1Widget::convert);
}

QString Task1Widget::intToBin(long long n)
{
    if (n == 0) return "0";
    if (n == 1) return "1";
...
void Task1Widget::fracToBin(double frac, int maxDigits, QString &out)
{
    if (maxDigits <= 0 || frac == 0.0) return;
    fracToBinHelper(frac, maxDigits, out);
}

void Task1Widget::fracToBinHelper(double frac, int maxDigits, QString &out)
{
    frac *= 2.0;
    if (frac >= 1.0) {
        out.append('1');
