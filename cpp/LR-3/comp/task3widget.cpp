    ...
    : QWidget(parent)
{
    ...
    connect(btn, &QPushButton::clicked, this, &Task3Widget::reverseNumber);
}

int Task3Widget::reverseInt(int n, int acc)
{
    if (n == 0)
        return acc;
...
bool Task3Widget::hasNoZero(int n)
{
    if (n == 0)
        return true;
    if (n % 10 == 0)
...
void Task3Widget::reverseNumber()
{
    bool ok = false;
